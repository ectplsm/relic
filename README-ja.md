...
v0.3.0 より前のバージョンでは、著作権のあるキャラクター名を使用したサンプルEngram（`johnny`, `motoko`）が同梱されていました。これらはオリジナルのペルソナ（`rebel`, `commander`）に置き換えられています。`refresh-samples` を実行して新しいサンプルを追加してください — 既存のEngramは **削除されません**:

```bash
relic refresh-samples
# → Seeded: 2 (commander, rebel)
# → johnny/motoko はそのまま残ります
```

新しいサンプルの動作を確認した後、不要であれば旧サンプルを削除できます:
...
> **既存ユーザーの方へ:** `relic refresh-samples` を実行して新しいサンプル人格を追加してください。旧サンプル（`johnny` / `motoko`）をお使いの方は、[マイグレーション](#マイグレーション) の置き換え手順もご確認ください。
