...
| ワークスペース側のペルソナ状態 | オプション | 挙動 |
|---------|------|------|
| ワークスペース未作成 | なし | エラーになり、先にエージェント作成が必要 |
| ローカルEngramと同一 | なし | ペルソナ再書き込みをスキップし、その後メモリsync |
| ローカルEngramと差分あり | なし | ペルソナ上書き前に確認を出す |
| ローカルEngramと差分あり | `--yes` | 確認なしでペルソナを上書き |
...
|---------|------|------|
| 未作成 | なし | ワークスペースの内容から新規Engramを作成 |
| 未作成 | `--force` | 通常の新規extractと同じ |
| 既存あり | なし | エラーになり、`--force` が必要 |
| 既存あり・ペルソナ差分なし | `--force` | 上書きをスキップ |
| 既存あり・ペルソナ差分あり | `--force` | `SOUL.md` / `IDENTITY.md` の上書き前に確認を出す |
| 既存あり・ペルソナ差分あり | `--force --yes` | 確認なしで `SOUL.md` / `IDENTITY.md` を上書き |

補足:
- ペルソナとは `SOUL.md` と `IDENTITY.md` の総称です
- `--force` で上書きされるのは `SOUL.md` と `IDENTITY.md` のみ
- `--force` でも `USER.md`、`MEMORY.md`、`memory/*.md` は上書きしない
- `--force` と `--name` を併用した場合は、`engram.json.name` も更新される
