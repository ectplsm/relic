...
Relicは [OpenClaw](https://github.com/openclaw/openclaw) のワークスペースと完全互換です。EngramをOpenClawエージェントに注入したり、既存のエージェントをEngramとして抽出できます。

### Inject — EngramをOpenClawに注入

```bash
# メインエージェントに注入（シングル/マルチを自動検出）
...
### Extract — OpenClawからEngramを作成

```bash
# メインエージェントを抽出（IDはデフォルトで"main"）
relic extract --name "My Agent"

# 特定のエージェントを抽出（IDはデフォルトでエージェント名）
...
Relicは OpenClaw のワークスペースモードを自動検出します:

| 条件 | `--agent` 省略時 | `--agent <name>` 指定時 |
|------|-----------------|----------------------|
| `~/.openclaw/agents/` が存在 | `agents/main/agent/` を対象 | `agents/<name>/agent/` を対象 |
| `~/.openclaw/agents/` が不在 | `workspace/` を対象 | エラー: agent not found |
